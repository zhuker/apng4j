...
class APNGWriter {
    ...
    public void write(Gray[] grays, FileOutputStream fos, int loopCount) throws IOException {
        if (grays.length <= 0) {
        ...
        try (WritableByteChannel out = fos.getChannel()) {
            ...
            for (int i = 0, seq = 0; i < grays.length; i++) {
                ...
                Gray gray = grays[i];
                short[] delay = getFractionFromDelay(gray.getDelay());

                out.write(makeFCTL(gray.width, gray.height, seq++, delay[0], delay[1]));
     ...
     */
    private short[] getFractionFromDelay(int delayms) {
        double x = delayms;
        x /= 1000;
    ...
    protected ByteBuffer make_acTLChunk(int frameCount, int loopCount) {
        ...
        bb.putInt(loopCount);           // loop count, zero=infinity
        addChunkCRC(bb);
        bb.flip();
        return bb;
    }
